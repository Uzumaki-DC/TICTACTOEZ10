...
export function useSupabaseRealtime(sessionCode: string | null) {
  ...
  const [connectionType, setConnectionType] = useState<'realtime' | 'polling' | 'none'>('none');
  ...
  const startPolling = useCallback(() => {
    ...
    const poll = async () => {
      try {
        const response = await apiRequest('GET', `/api/sessions/${sessionCode}/poll`);
  ...
  const joinSession = useCallback(async (playerId: string) => {
    ...
    try {
      console.log('Joining session:', sessionCode, playerId);
